...
"""Export state data for a given episode from a LeRobotDataset to a numpy file."""
...
def export_episode_actions(
    ...
    """Export states from a single episode to a .npy file."""
    ...
    states = []
    indices = []
    for batch in tqdm.tqdm(dataloader, desc="Extracting states"):
        if "observation.state" in batch:
            states.append(batch["observation.state"])
            indices.append(batch["index"])

    if not states:
        logging.warning("No states found in the episode.")
        return

    # Sort states by index to ensure correct order, which is important
    ...
    states_np = sorted_states.numpy()

    output_path.parent.mkdir(parents=True, exist_ok=True)
    np.save(output_path, states_np)
    logging.info(f"Successfully saved {len(states_np)} states to {output_path}")


def main():
    parser = argparse.ArgumentParser(
        description="Export state data from a LeRobotDataset episode to a .npy file for Mujoco replay."
    )
    parser.add_argument(
        "--repo-id",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Episode to export states from.",
    )
    parser.add_argument(
        "--output-path",
        type=Path,
